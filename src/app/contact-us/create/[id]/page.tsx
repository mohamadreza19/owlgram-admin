  ...
  return (
    <form onSubmit={formik.handleSubmit} className="container">
      ...
      {socialNetworks?.map((socialNetwork, index) => {
        return (
          <div key={index} className="w-full flex flex-col gap-y-3 mb-3">
            ...
            <TextInput
              onChange={(e) => {
                ...
                const findIndex = formik.values.socialNetworks.findIndex(
                  (s) => s.social_id === socialNetwork.id
                );

                if (findIndex >= 0) {
                  const social_networks = [...formik.values.socialNetworks];

                  social_networks.splice(findIndex, 1, social_network);

                  formik.setFieldValue("socialNetworks", social_networks);
                } else {
